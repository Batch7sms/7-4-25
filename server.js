...
app.post("/api/upgrade-semester", (req, res) => {
    const { branch_id, excludedRollNumbers } = req.body;
    ...
    db.query(branchQuery, [branch_id], (err, branchResult) => {
        ...
        // Upgrade students' semester (except semester 8)
        const sql = `
            UPDATE students 
        ...
        const params = [branch_id, ...excludedRollNumbers];
        ...
        db.query(sql, params, (err, result) => {
            ...
            const updateSql = `
                INSERT INTO semester_updates (branch_id, branch_name, excluded_roll_numbers)
                VALUES (?, ?, ?)
            `;
            const excludedStr = excludedRollNumbers.length ? excludedRollNumbers.join(", ") : "None";

            db.query(updateSql, [branch_id, branchName, excludedStr], (err) => {
                if (err) return res.status(500).json({ error: err.message });
...
app.get("/api/get-last-updated", (req, res) => {
    ...
    let sql = `SELECT branch_name, excluded_roll_numbers, DATE_FORMAT(updated_at, '%Y-%m-%d %H:%i:%s') AS updated_at FROM semester_updates`;
    let params = [];
    ...
    db.query(sql, params, (err, result) => {
        ...
        const { branch_name, excluded_roll_numbers, updated_at } = result[0];
