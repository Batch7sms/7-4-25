...
function renderPagination(totalUsers) {
    ...
    paginationDiv.innerHTML = `
        ...
        Page ${currentPage}
        <button onclick="changePage(1)" ${currentPage * usersPerPage >= totalUsers ? "disabled" : ""}>
            <i class="fas fa-arrow-right"></i>
        </button>
    `;
}
...
document.getElementById("upgradeSemesterBtn").addEventListener("click", async function () {
    ...
    if (!branch_id) return alert("Branch ID is required!");
    ...
    const response = await fetch("http://localhost:5000/api/upgrade-semester", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ branch_id, excludedRollNumbers })
    });
